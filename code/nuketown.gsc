...
init_custom_map()
{
	//perk machine niketown
	perk_system( "script_model", (  -638, 268, -54 ), "zombie_vending_jugg_on", ( 0, 165, 0 ), "random", "mus_perks_stamin_sting", "Random Perk", 2500, "sleight_light" );
}
        ...
        foreach( player in level.players )
        {
            if(!player.machine_is_in_use)
			{
                if( distance( self.origin, player.origin ) <= 70 )
                {
					...
					if( type == "random" && !player.num_perks > 14 && player usebuttonpressed() && ( player.score >= cost ) && !player maps/mp/zombies/_zm_laststand::player_is_in_laststand())
					{
					...
					if(type == "random" && player.num_perks > 14 && player usebuttonpressed() && ( player.score >= cost ) )
					{
...
drawshader_and_shadermove(perk, custom, print)
{
	 ...
	 for(i = 0; i < self.perkarray.size; i++)
	{
        ...
        if(perk == "Downers_Delight")
        {
            ...
            self.perk1front = self drawshader( "waypoint_revive", x, -40, 23, 23, ( 2, 2, 1 ), 100, 0 ); 
		...
		if(perk == "Ammo_Regen")
        {
            ...
            self.perk8front = self drawshader( "menu_mp_lobby_icon_customgamemode", x, -40, 23, 23, ( 2, 2, 2 ), 100, 0 );
        ...
        if(perk == "Dying_Wish")
        {
            ...
            self.perk10front = self drawshader( "zombies_rank_5", x, -40, 23, 23, ( 9, 9, 9 ), 100, 0 );
...
give_random_perk()
{
   ...
   if(!self hasPerk("specialty_armorvest"))
    ...
    perk = n[0];
    if(perk == "specialty_armorvest" || perk == "specialty_rof" || perk == "specialty_fastreload" || perk == "specialty_longersprint" || perk == "specialty_quickrevive" || perk == "specialty_scavenger")
	{
